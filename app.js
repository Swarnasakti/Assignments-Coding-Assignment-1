...
const toDate = require("data-fns/toDate");
...
const checkRequestsQueries = async (request, response, next) => {
 ...
 const checkRequestsBody = (request, response, next) =>{
    ...
    if (priority !== undefined){
        priorityArray = ["HIGH","MEDEIUM","LOW"];
    ...
    app.get("/todos/", checkRequestsQueries,async(request,response) => {
        const {status = "", search_q = "",priority,category};
        ...
        const getTodasQuery = `
            ...
            FROM 
                 todo
            WHERE 
            todo LIKE '%${seach_q}%' AND priority LIKE '%${priority}%'
